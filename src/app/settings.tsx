...
import { savePrinterSettings, getPrinterSettings, PrinterSettings } from '../utils/storage';
...
export default function SettingsScreen() {
  ...
  const handleSaveSettings = async () => {
    ...
    if (!validateIP(ipAddress.trim())) {
      Alert.alert('Erro', 'Por favor, insira um endereço IP válido (ex: 192.168.1.100).');
      return;
    }
  ...
  const handleTestPrint = async () => {
    if (!ipAddress.trim() || !port.trim()) {
      Alert.alert('Erro', 'Por favor, preencha o IP e a porta antes de testar.');
      return;
    }

    if (!validateIP(ipAddress.trim()) || !validatePort(port.trim())) {
      Alert.alert('Erro', 'Por favor, verifique se o IP e a porta estão válidos.');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Erro no teste', error.message || 'Falha ao executar teste de impressão.');
    } finally {
      setIsTesting(false);
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.formContainer}>
          ...
          <View style={styles.inputContainer}>
            ...
            <View style={styles.pickerContainer}>
              <Picker
                ...
                style={styles.picker}
              >
                <Picker.Item label="ESC/POS" value="ESC/POS" />
                <Picker.Item label="ZPL (Zebra)" value="ZPL" />
                <Picker.Item label="EPL (Eltron)" value="EPL" />
              </Picker>
            </View>
          </View>
          ...
          <View style={styles.buttonContainer}>
            ...
            >
              <Text style={styles.secondaryButtonText}>Limpar Configurações</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={styles.infoContainer}>
          ...
          <Text style={styles.infoText}>
            1. Certifique-se de que a impressora está conectada à mesma rede Wi-Fi{'\n'}
